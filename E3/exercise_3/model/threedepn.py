...
class ThreeDEPN(nn.Module):
    def __init__(self):
        ...
        self.encoder1 = nn.Sequential(conv1,relu1)
        self.encoder2 = nn.Sequential(conv2,batch2,relu1)
        self.encoder3 = nn.Sequential(conv3,batch3,relu1)
        self.encoder4 = nn.Sequential(conv4,batch4,relu1)
        ...
        conv1 = nn.ConvTranspose3d(in_channels=self.num_features * 8 * 2,out_channels=self.num_features*4*2,  kernel_size=4,stride=2,padding=1)
        batch_norm1 = nn.BatchNorm3d(self.num_features * 4 *2)
        conv2 = nn.ConvTranspose3d(in_channels=self.num_features * 4 * 2,out_channels=self.num_features*2*2,  kernel_size=4,stride=2,padding=1)
        batch_norm2 = nn.BatchNorm3d(self.num_features * 2 *2)
        conv3 = nn.ConvTranspose3d(in_channels=self.num_features * 2 * 2,out_channels=self.num_features*2,  kernel_size=4,stride=2,padding=1)
        batch_norm3 = nn.BatchNorm3d(self.num_features * 2)
