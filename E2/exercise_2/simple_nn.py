...
class SimpleDataset(torch.utils.data.Dataset):
    ...
    def __init__(self,split="train"):
        samples = TRAIN_N_SAMPLES if split=="train" else VAL_N_SAMPLES
        
        self.sdfs,self.labels = generate_toy_data(samples)
      

    def __getitem__(self, idx):
        # or use expand_dims
        sdf = self.sdfs[idx][np.newaxis,:,:,:]
...
def main(config):
    ...
    # Create Dataloaders
    # Youssef: Why are we splitting manually ??, there are predefined splits in pytorch
