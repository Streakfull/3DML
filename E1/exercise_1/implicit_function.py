...
def signed_distance_sphere(x, y, z, r, x_0, y_0, z_0):
    ...
    y_centered = y - y_0
    z_centered = z - z_0
    point = np.array([x_centerd, y_centered, z_centered],dtype=np.float64)
    points_squared = point **2
    distance = np.sum(points_squared,axis=0)
    distance = np.sqrt(distance) - r 
    #square_root = np.sqrt(points_squared)
    #distance = np.sum(square_root, axis=0) - r
    #distance[np.abs(distance) < 1e-15] = 0
    return distance
    # ###############
...
def signed_distance_torus(x, y, z, R, r, x_0, y_0, z_0):
    ...
    y_centered = y - y_0
    z_centered = z - z_0
    x_plus_z = np.array([x_centerd,z_centered])**2
    a = np.sum(x_plus_z,axis=0)
    a = np.sqrt(a) - R
    points = np.array([a,y_centered])
...
def signed_distance_atom(x, y, z):
    ...
    all_sdf = np.array([proton_sdf,orbit_sdf,electron_sdf])
    union = np.min(all_sdf,axis=0)
    # ###############
    # TODO: Implement
    return union
    # ###############
